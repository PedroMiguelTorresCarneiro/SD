...
import serverSide.entities.IDCheckClientProxy;
...
public class ServerIDCheck {
   ...
   */
   ...
   {
      IDCheck iDCheck;                                              // barber shop (service to be rendered)
      IDCheckInterface iDCInter;                              // interface to the barber shop
     ...
     /* service is established */
      ...
      iDCheck = new IDCheck(reposStub);                                      // service is instantiated
      iDCInter = new IDCheckInterface(iDCheck);                            // interface to the service is instantiated
     ...
     /* service request processing */
      ...
      IDCheckClientProxy cliProxy;                                // service provider agent

      waitConnection = true;
      while (waitConnection)
      { try
        { sconi = scon.accept ();                                    // enter listening procedure
          cliProxy = new IDCheckClientProxy(sconi, iDCInter);    // start a service provider agent to address
